...
def direct_from_stills(still_urls, sb, config, include_human, product_urls=None,
    ...
    prompt = (
        ...
        f"way, set pass=false with a short issue. PRINTED TEXT: the reference "
        f"photo's own printed text is the only truth - lettering on the "
        f"product that is not in the reference, or garbled/unreadable "
        f"lettering, is a FAIL; but NEVER require the advertiser's brand name "
        f"to appear on the product.\n"
