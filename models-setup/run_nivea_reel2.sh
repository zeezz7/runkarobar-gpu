...
cd "$V"; : > nvconcat2.txt
for i in 5 6 7 8; do echo "file '$V/nv${i}_clip_00001_.mp4'" >> nvconcat2.txt; done
ffmpeg -loglevel error -y -f concat -safe 0 -i nvconcat2.txt -c copy nivea2_master.mp4
ffmpeg -loglevel error -y -i nivea2_master.mp4 -vf "scale=1080:1920:flags=lanczos,format=yuv420p" \
  -c:v libx264 -profile:v high -preset medium -crf 21 -maxrate 6M -bufsize 12M -movflags +faststart -r 30 nivea_reel2_1080p.mp4
ffmpeg -loglevel error -y -i nivea2_master.mp4 -vf "scale=720:1280:flags=lanczos,format=yuv420p" \
  -c:v libx264 -profile:v high -preset medium -crf 23 -maxrate 2M -bufsize 4M -movflags +faststart -r 30 nivea_reel2_720p.mp4
ls -la nivea_reel_*.mp4 | awk '{printf "  %-26s %6.1f MB\n",$9,$5/1e6}'

echo "=== STAGE 4: upload ==="
cd /workspace/models-setup
for i in 5 6 7 8; do $PY minio_upload.py /workspace/ComfyUI/output/nivea_scene${i}.png --prefix images 2>&1 | tail -1; done
for f in nivea_reel2_720p nivea_reel2_1080p; do $PY minio_upload.py "$V/${f}.mp4" --prefix reels 2>&1 | tail -1; done
echo "NIVEA_REEL2_DONE"
